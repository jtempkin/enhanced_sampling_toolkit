...
def main():
    ...
    params['scratchdir'] = "/project/weare-dinner/jtempkin/enhanced_sampling_toolkit/neus/debug_US"
    params['inputFilename'] = "/project/weare-dinner/jtempkin/enhanced_sampling_toolkit/neus/input.diala"
    ...
    for i in range(len(system.umbrellas)):
        ...
        for j in range(data.shape[0]):
            ...
            system.umbrellas[i].entryPoints.append(data[j])
            
        
    
    # sanity check that we've actually loaded something here
    for i in range(len(system.umbrellas)):
        #print len(system.umbrellas[i].entryPoints)
        if len(system.umbrellas[i].entryPoints) == 0:
            print "WARNING: No entry points loaded for umbrella ", i
    ...
    for i in range(len(system.umbrellas)):
        ...
        wlkr = lammpsWalker.lammpsWalker(params['inputFilename'], params['logFilename'], index=i, debug=False)
        ...
        """
        for i in range(66):
            print system.umbrellas[i].entryPoints[temp_indx][i]
           """ 
