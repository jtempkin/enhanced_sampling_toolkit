...
class Box(Window):
    """Class definition of Box which inherits from window. Implements the Box shaped basis function.
    """
    ...
    def __init__(self, center, width, ref_center=None, ref_width=None, time=None, periodic_length=None, max_list_size=100, initial_conditions=[], initial_conditions_probability=0.0):
        """Create an intsance of the Box object.
        ...
        width : numpy.ndarray, list
            The width of the support of the window in nx2 size. First column is witdh in negative direction, second column is width in positive direction.
        ...
        # We calculate the slope of the Box.
        #self.slopes = 1.0/self.width
        self.w0 = self.center - self.width[0]
        self.w1 = self.center + self.width[1]

        return None

    def __repr__(self):
        """Return the string represetation of the Box instance.

        Returns
        ----------
        string
            A string representation of the Box instance.
        """
        id = "Box(" + str(self.center) + ", " + str(self.width) + ")"

        return id

    def __call__(self, walker):
        """Return the value of the support for this Box object.

        Parameters
        -------------
        walker : walker instance
            The walker instance for which to evaluate the support of the Box.
        ...
        # ok, here we are going to implement the window on top if the walker object definition. But we will enforce that the behavoir of Box's call will depend on both the definition of the reference and the information it can get from walker.
        ...
        val = 1.0 * np.all(self.w0 < coord) and np.all(self.w1 > coord)
        ...
        if self.ref_center is not None:
            ...
            if not hasattr(walker, "get_initial_colvars"):
                raise Exception("Walker object passed to Box __call__ does not have support for getting refernce collective variable value.")
        ...
        if self.time_start is not None:
            ...
            if not hasattr(walker, "get_time"):
                raise Exception("Walker object passed to Box __call__ does not have support for getting time value.")
